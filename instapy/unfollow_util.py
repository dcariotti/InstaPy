...
def unfollow_user(browser, track, username, person, person_id, jump4xfollowing, inactive, button, relationship_data, logger, logfolder):
    ...
    if inactive[0] or jump4xfollowing:
        import urllib.request
        from bs4 import BeautifulSoup as bs4
        try:
            import re
            p = urllib.request.urlopen(urllib.request.Request('https://www.instagram.com/{}/'.format(person)))
            b = bs4(p, 'html.parser')
            x = b.findAll('script')[4]
            pattern = json.loads(re.search('_sharedData = (.+?);</script>', str(x)).group(1))
            user_pattern = pattern['entry_data']['ProfilePage'][0]['graphql']['user']
            if inactive[0]:
                last_taken_at = user_pattern['edge_owner_to_timeline_media']['edges'] [0]['node']['taken_at_timestamp']
                if datetime.now() - datetime.fromtimestamp(last_taken_at) < timedelta(days=inactive[1]):
                    logger.warning('--> {} : Utente attivo da più di {} giorni'.format(person, inactive[1]))
                    return True, 'non rispecchia i criteri di inattività per essere unfollowato'
            if jump4xfollowing:
                followers, followings = int(user_pattern['edge_followed_by']['count']), int(user_pattern['edge_follow']['count'])
                if followings <= followers*4:
                    logger.warning('--> {} : Utente con numero di followings inferiore 4 volte ai followers'.format(person))
                    return True, 'numero di followings 4x ai followers'
