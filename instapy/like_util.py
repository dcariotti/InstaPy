...
def check_link(browser, post_link, dont_like, mandatory_words, ignore_if_contains, logger, tags2=False, locations=False):
    ...
    if tags2:
        ...
        if locations and check_for_tags2:
            p = urllib.request.urlopen(urllib.request.Request(post_link, headers={'User-Agent': 'Mozilla/5.0'}))
            b = bs4(p, 'html.parser')
            x = b.findAll('script')[3]

            try:
                loc = re.search('"location":{"id":"(.+?)"', str(x)).group(1)
            except:
                return False, user_name, is_video, 'None', "Success", False

            return False, user_name, is_video, 'None', "Success", loc in locations
